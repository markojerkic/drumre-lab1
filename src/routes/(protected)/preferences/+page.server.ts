...
export const load: PageServerLoad = async (event) => {
	const bookGenres = books.distinct("volumeInfo.categories").then((categories: string[]) => {
	...
	const [g1, g2, userGenres] = await Promise.all([bookGenres, showGenres, userSavedGenres]);
...
export const actions: Actions = {
	default: async ({ request, locals }) => {
		const selectedGenres = await request.formData().then((data) => data.getAll("genres"));
